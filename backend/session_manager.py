...
class SessionManager:
    ...
    def create_session(self) -> str:
        ...
        session_data = {
            ...
            'session_folder': os.path.join(self.base_sessions_dir, f"session_{timestamp}_{session_id[:8]}"),
            'upload_folder': None,
        ...
        session_data['upload_folder'] = os.path.join(session_data['session_folder'], 'uploads')
        session_data['processed_folder'] = os.path.join(session_data['session_folder'], 'processed')
        session_data['inverted_folder'] = os.path.join(session_data['session_folder'], 'inverted')
        session_data['tps_folder'] = os.path.join(session_data['session_folder'], 'tps')
        session_data['image_download_folder'] = os.path.join(session_data['session_folder'], 'annotated')
        session_data['outputs_folder'] = os.path.join(session_data['session_folder'], 'outputs')
        ...
        logger.info(f"Created new session: {session_id} at {session_data['session_folder']}")
    ...
    def clear_session(self, session_id: str) -> Dict:
                    ...
                    for root, dirs, files in os.walk(session_folder):
                        for file in files:
                            ...
                            try:
                                ...
                                logger.warning(f"Could not delete {file_path}: {str(file_error)}")
                                errors.append(f"Could not delete {file}")
                    
                    cleared_items.append(f"{item_count} files from session {session_id[:8]}")
        ...
        try:
            ...
            if root_files_cleared > 0:
                cleared_items.append(f"{root_files_cleared} output files from backend root")
    ...
    def list_sessions(self) -> List[Dict]:
        ...
        for folder_name in os.listdir(self.base_sessions_dir):
            if folder_name.startswith('session_') and os.path.isdir(os.path.join(self.base_sessions_dir, folder_name)):
