...
for folder in [UPLOAD_FOLDER, COLOR_CONTRAST_FOLDER, TPS_DOWNLOAD_FOLDER, 
               IMAGE_DOWNLOAD_FOLDER, INVERT_IMAGE_FOLDER, OUTPUTS_FOLDER]:
    ...
    try:
        ...
        for image in images:
            if image:
                ...
                processed_path = os.path.join(COLOR_CONTRAST_FOLDER, f'processed_{unique_name}')
                inverted_path = os.path.join(INVERT_IMAGE_FOLDER, f'inverted_{unique_name}')
...
def process_scatter_data():
    ...
    try:
        ...
        output_paths = visual_individual_performance.create_image(tps_file_path, IMAGE_DOWNLOAD_FOLDER)
...
def process_existing():
    try:
        ...
        if not os.path.exists(xml_path):
            utils.predictions_to_xml_single("better_predictor_auto.dat", image_path, xml_path)
            utils.dlib_xml_to_pandas(xml_path)
            utils.dlib_xml_to_tps(xml_path)
        
        # Copy all files to export directory
        export_handler.copy_file_to_export(image_path, export_dir)
        export_handler.copy_file_to_export(processed_path, export_dir, f"processed_{filename}")
        export_handler.copy_file_to_export(inverted_path, export_dir, f"inverted_{filename}")
        export_handler.copy_file_to_export(xml_path, export_dir, os.path.basename(xml_path))
        
        # Copy CSV and TPS files
        csv_path = f"output_{filename}.csv"
        tps_path = f"output_{filename}.tps"
        if os.path.exists(csv_path):
            export_handler.copy_file_to_export(csv_path, export_dir, os.path.basename(csv_path))
        if os.path.exists(tps_path):
            export_handler.copy_file_to_export(tps_path, export_dir, os.path.basename(tps_path))
